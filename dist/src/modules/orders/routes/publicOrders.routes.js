...
import { createExternalOrderBodySchema, externalOrderHeadersSchema } from "../contracts/createExternalOrder.contract.js";
import { OrderIngestionService } from "../services/orderIngestion.service.js";
export const registerPublicOrdersRoutes = (app, repository) => {
    ...
    app.get("/api/public/integraciones/pedidos/orders/:orderId", { preHandler: authGuard.preHandler }, async (request, reply) => {
        const context = request.integrationContext;
        if (!context) {
            throw new Error("Missing integration context");
        }
